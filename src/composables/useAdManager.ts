...
export function useAdManager(config: AdConfig) {
  ...
  const AD_GROUPS = {
    ...
    group5: [
      '20248264', // 保价50
      '20248284', // 竞价(setbid:2000)
    ],
  }; // 共14个广告位（静序念经D版，含3个竞价位）
  ...
  const generateSimulatedEcpm = (slotId: string): number => {
    const ecpmRanges: { [key: string]: [number, number] } = {
      ...
      '20261921': [180, 200],    // 竞价(setbid:28000)
      ...
      '20261914': [90, 100],     // 竞价(setbid:10000)
      '20248256': [72, 80],      // 保价80
      // group5 - 保价50, 竞价
      '20248264': [45, 50],      // 保价50
      '20248284': [20, 30],      // 竞价(setbid:2000)
    };
  ...
  const preloadNextAd = async (): Promise<void> => {
    ...
    isPreloading = true;
    console.log('🚀 开始预加载任务（策略：一轮轮询所有14个广告位）');
