...
export function useAdManager(config: AdConfig) {
  ...
  const AD_GROUPS = {
    group5: [
      '19281661', '19281662', '19281663', // 保价1500、1000、900
      '19281665', '19281671', '19281677', // 保价800、700、600
  ...
  const isBiddingSlot = (slotId: string): boolean => {
    const biddingSlots = ['19281711', '19281723', '19281746'];
  ...
  const preloadNextAd = async (): Promise<void> => {
    ...
    preloadingPromise = (async () => {
      ...
      for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
        ...
        // 阶段2：中间9个广告位，每3个一组并行请求
        if (!foundAd) {
          const middleSlots = slotIds.slice(3, 12); // 中间9个广告位
        ...
        // 阶段3：最后6个广告位串行
        if (!foundAd) {
          const lastSerialSlots = slotIds.slice(12); // 最后6个广告位
